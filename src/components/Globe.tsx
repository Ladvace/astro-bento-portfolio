...
type RenderOptions = {
  ...
  visitedCountries: typeof SITE.visitedCountries;
};
...
function renderGlobe(
  ...
  const width = Math.max(container.clientWidth, minDim);
  const height = Math.max(container.clientHeight || FALLBACK_HEIGHT, minDim);
  const cx = width / 2;
  const cy = height / 2;
  ...
  const orientFactor = WAVE_SCALE / (ORIENT_EPS_DEG * DEG) / projection.scale();
  ...
  const updateWaves = () => {
    ...
    waveElements.each(function (d) {
      ...
      if (geoDistance([d.lon, d.lat], viewCenter) > halfPi) {
  ...
  const featureCull = features.map((f) => {
    ...
    forEachCoord(f.geometry.coordinates, (p) => {
      const r = geoDistance(centroid, p);
  ...
  const tickMs = enableHover ? 0 : TICK_MS;
  const degreesPerTick =
    (DRAG_SENSITIVITY / DEFAULT_PROJECTION_SCALE) *
    Math.max(1, tickMs / FRAME_MS);
  ...
  return runWhileVisible(
    ...
    () => {
      ...
      const rot = projection.rotate();
      projection.rotate([rot[0] - degreesPerTick, rot[1]]);
      updatePaths();
    },
    tickMs,
  );
...
const GlobeComponent = ({ isStatic, enableHover, scale }: Props) => {
  ...
  onMount(() => {
    ...
    void loadFeatures().then((features) => {
      if (disposed) return;
      stop = renderGlobe(container, features, {
        isStatic,
        enableHover,
        projectionScale: scale ?? DEFAULT_PROJECTION_SCALE,
        visitedCountries: SITE.visitedCountries,
      });
    });
  });
