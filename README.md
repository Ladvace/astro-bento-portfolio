...
- 💬 **Guestbook** — visitors can leave messages and reactions (backed by Astro DB)
...
| Database | [Astro DB](https://docs.astro.build/en/guides/astro-db/) on [libSQL / Turso](https://turso.tech/) (guestbook) |
...
- Swap the avatar/memoji images — replace `src/assets/me*.webp` with your own.
...
| `pnpm build` | Production build (`astro build --remote`, uses the remote DB) |
...
The guestbook uses **Astro DB** backed by a remote **libSQL** database (e.g. [Turso](https://turso.tech/)). Provide your own database credentials as environment variables:
...
> The `build` script runs with `--remote`, so these variables must be set in your deploy environment. If you don't need the guestbook, you can remove the `@astrojs/db` integration and the `guestbook`/`reactions` routes.
